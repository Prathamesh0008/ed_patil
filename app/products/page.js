...
/**
 * Helper function – get the correct unit price based on quantity and product pricing tiers.
 * @param {Object} product - The product object containing a `pricing` array.
 * @param {number} qty - The quantity selected.
 * @returns {number} - The applicable unit price.
 */
const getPriceByQty = (product, qty) => {
  if (!product?.pricing || !Array.isArray(product.pricing)) return 0;
  const tier = product.pricing.find(
    (p) => qty >= p.min && qty <= p.max
  );
...
const getAllProducts = () => {
  return Object.values(products).map((p) => ({
    ...
    pricing: p.pricing || [],
    // Static placeholders – adjust as needed
    rating: 4.5,
    stock: 50,
  }));
};
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="mt-auto space-y-3">
          ...
          <Link
            ...
            className="group relative w-full text-center py-3.5 rounded-xl font-bold hover:scale-[1.02] transition-all duration-300 overflow-hidden block"
          >
        ...
        >
          <div
            className={`w-3 h-3 rounded-full transition-all duration-300 ${
              selected === 'All'
                ? 'bg-white scale-125'
                : 'bg-gradient-to-r from-[#2596be]/50 to-[#122E34]/50'
            }`}
          ></div>
          <span className="font-medium">All</span>
          {selected === 'All' && (
            <svg
              className="w-4 h-4 ml-auto opacity-80 animate-pulse"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
